...
type TimelineGroupItemProps = {
  ...
  onMeasure: (index: number, node: HTMLDivElement | null) => void;
};
...
const TimelineGroupItem = memo(function TimelineGroupItem({
  ...
  const refCallback = useCallback(
    (node: HTMLDivElement | null) => {
      onMeasure(absoluteIndex, node);
    },
    [absoluteIndex, onMeasure]
  );
...
const App = () => {
  ...
  const mergeTimeline = (prev: ReleaseGroup[], entries: ReleaseGroup[]) => {
    ...
    // Preserve object references for unchanged groups to help React.memo and reduce allocations.
    prev.forEach((entry) => {
    ...
    entries.forEach((entry) => {
      ...
      const mergedGames = existing.games.length
        ? [...existing.games, ...entry.games]
        : [...entry.games];
      const nextDisplayDate = entry.displayDate || existing.displayDate || entry.date;
      grouped.set(entry.date, {
        date: existing.date,
            ...
            {showError ? (
              ...
              visibleGroups.map((group, index) => {
                ...
                const animationDelay = animationsEnabled
                  ? Math.min(index * ANIMATION_STAGGER_STEP, ANIMATION_MAX_DELAY)
                  : undefined;
